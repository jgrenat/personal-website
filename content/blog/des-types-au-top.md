...
Prenons maintenant un peu de recul. Pour décrire le type `Color` en toutes lettres, on pourrait dire ceci :
 ...
 > `Color` est un type qui peut valoir `HEARTS` **ou** `CLUBS` **ou** `SPADES` **ou** `DIAMONDS` **ou** `NEW_COLORS`.
...
Les langages supportant les *types algébriques* sont surtout des langages fonctionnels. On peut penser à Rust, Haskell, Scala, OCaml, Haskell, Elm, ReasonML, etc. Certains les utilisent plus ou moins en [*Kotlin*](https://proandroiddev.com/algebraic-data-types-in-kotlin-337f22ef230a) également.
...
```java
public enum GiftWrap {
	NO_GITF_WRAP, CHILD_GIFT_WRAP, ADULT_GIFT_WRAP;
}
```
...
Et mon dernier conseil est sans doute le plus important : essayez de rendre possible uniquement les valeurs que votre métier accepte ; éliminez grâce aux types le maximum d'états impossibles. En ce sens, je vous recommande cet excellent talk de Richard Feldman : [Making impossible states impossible](https://www.youtube.com/watch?v=IcgmSRJHu_8).